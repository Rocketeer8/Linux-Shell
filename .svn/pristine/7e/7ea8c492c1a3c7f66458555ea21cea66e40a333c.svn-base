...
/**
 * The PushDirectory class is a Command that pushes the working directory onto
 * the directory stack then changes the new working directory to a given
 * directory.
 * 
 * @author Aliel
 */
public class PushDirectory extends Command {
  ...
  /**
   * Constructs a new PushDirectory with its valid argument counts and
   * documentation.
   *
  ...
  /**
   * If the size of arguments is in validArgumentCounts then this pushes
   * workingDirectory onto the directoryStack, sets dir as the new
   * workingDirectory, and returns dir as a Directory. Otherwise, returns null.
   *
   * @param shellInput An array of arguments for the command.
   * @return Dir, as the new working directory, or null.
   */
  @Override

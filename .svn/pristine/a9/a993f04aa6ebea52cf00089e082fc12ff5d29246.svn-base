...
public class SearchTest {
  ...
  /**
   * Tests execute with an invalid expression name.
   */
  @Test
  public void testExecuteWithInvalidExpressionName() {
    // Checks if -type is in the right location
    String args = "invalid_expression -type d -name \"express\"ion\"";
    assertEquals("Inv Expression", search.execute(new String[] {args}));
  }

  /**
   * Tests execute with missing required arguments. That is, it contains -type
   * [f|d] -name "expression" as the last indices in the argument in that order.
   */
  @Test
